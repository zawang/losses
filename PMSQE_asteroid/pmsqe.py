...
class SingleSrcPMSQE(nn.Module):
    ...
    """
    def __init__(self, device, window_name='sqrt_hann', window_weight=1.0,
...
#        print(d_frame.requires_grad)
    ...
    def populate_constants(self, sample_rate):
        ...
        self.mask_sll = nn.Parameter(tensor(mask_sll, device=self.device), requires_grad=False)
    ...
    def register_16k_constants(self):
        ...
        abs_thresh_power = [
            ...
            0.416869, 0.537032]
        self.abs_thresh_power = nn.Parameter(tensor(abs_thresh_power, device=self.device),
                                             requires_grad=False)
        ...
        modif_zwicker_power = [
            ...
            0.23, 0.23, 0.23, 0.23, 0.23, 0.23, 0.23, 0.23, 0.23, 0.23, 0.23,
            0.23, 0.23, 0.23, 0.23]
        self.modified_zwicker_power = nn.Parameter(tensor(modif_zwicker_power, device=self.device),
                                                   requires_grad=False)
        # Width of band bark
        width_of_band_bark = [
            ...
            0.585232]
        self.width_of_band_bark = nn.Parameter(tensor(width_of_band_bark, device=self.device),
                                               requires_grad=False)
        # Bark matrix
        ...
        self.bark_matrix = nn.Parameter(tensor(bark_matrix, device=self.device),
                                        requires_grad=False)
